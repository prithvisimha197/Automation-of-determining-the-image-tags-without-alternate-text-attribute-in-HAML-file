...
class ImageTagAutomation
  ...
  def check_image_attributes(file_name, user_choice_param)
    File.foreach(file_name) do |line_iterator|
      if line_iterator.match(/%img\b/) || line_iterator.match(/(=)(\s*)?img\b/) || line_iterator.match(/[^#."'](\s)?image_tag\b/)
        if user_choice_param == 1 || user_choice_param == 3
          if !line_iterator.match(/(:)?(\s*)?alt(\s*)?(=>)?(:)?(\s*)?["'](.*)\b/)
            File.open('output_file.txt', 'a') do |f|
        ...
        unless user_choice_param == 2 || user_choice_param == 3 || user_choice_param == 1
          return -1
        end
      end
    end
  ...
  def get_path(lang_choice)
    ...
    # Appending "/**/*.haml" to path name provided by user to match the format
    # required for parsing the directories and to consider haml files only
